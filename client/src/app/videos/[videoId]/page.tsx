...
const DescriptionBox = ({
  ...
  return (
    <div className="mt-4 rounded-xl bg-neutral-100 p-4 dark:bg-neutral-800/50">
      ...
      <p
        className={`mt-2 text-sm text-neutral-600 dark:text-neutral-300 whitespace-pre-wrap ${
      ...
      <button
        ...
        className="mt-2 text-sm font-semibold text-neutral-800 dark:text-white hover:underline"
      >
...
const SuggestedVideoCard = ({ video }: { video: Video }) => (
  <a href={`/videos/${video._id}`} className="flex gap-3">
    <img
      src={video.thumbnail}
      alt={video.title}
      className="aspect-video w-40 flex-shrink-0 rounded-lg object-cover"
    />
    <div>
      <h4 className="font-semibold text-neutral-800 dark:text-neutral-100 line-clamp-2">
        {video.title}
      </h4>
...
const VideoPage = () => {
  ...
  useEffect(() => {
    if (video?.owner?.isSubscribed !== undefined) {
  ...
  const handleSubscribe = async () => {
    ...
    try {
      ...
      setIsSubscribed(true); // immediately update button
    } catch (err) {
